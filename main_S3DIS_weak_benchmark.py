"""
main entry for benchmarking weak supervision for RandLA-Net, reproduced based on the SQN paper, check https://arxiv.org/abs/2104.04891
Essentially, this weak supervision is almost similar to the official RandLA-Net except that a point cloud example does not have full labels, which are partially labeled.
- config same as RandLA-Net's config (but need add weak_label_ratio attribute)
- dataset preparation, add weak_labels for raw and sub_pc and same as preparing SQN dataset, check dataset_prepare_s3dis_sqn.py and main_S3DIS_Sqn.py
- model same as the RandLA-Net
- Loss: only compute losses of weakly points
- train and evaluation are the same
Author: Chao YIN
...
from helper_tool import ConfigS3DIS_Benchmark as cfg
from helper_tool import DataProcessing as DP
from helper_tool import Plot
import tensorflow as tf
# tf.enable_eager_execution()

class S3DIS_Benchmark:
    ...
    def get_tf_mapping2():
        ...
        def tf_map(batch_xyz, batch_features, batch_labels,batch_weak_label_mask, batch_pc_idx, batch_cloud_idx):
            ...
            input_list += [batch_features, batch_labels, batch_pc_idx, batch_cloud_idx, batch_weak_label_mask]
    ...
    if Mode == 'train':
        ...
        with tf.Session() as sess:
            ...
            while True:
                ...
                labels = flat_inputs[4*cfg.num_layers +1]
